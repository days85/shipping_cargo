...
func newDelivery(lastEvent HandlingEvent, itinerary Itinerary, rs RouteSpecification) Delivery {
	var (
		...
		isUnloadedAtDestination = calculateUnloadedAtDestinations(lastEvent, rs)
...
func calculateUnloadedAtDestinations(event HandlingEvent, rs RouteSpecification) bool {
	if event.Activity.Type == NotHandled {
		return false
	}
